...
def test_depth(opt):
    ...
    with open(test_files_list_path, 'r') as f:
        ...
        # t: 2011_09_26/2011_09_26_drive_0027_sync/image_02/data/0000000175.png\n"
        # [:-5] to remove ".npy\n"
        ...
        # '2011_09_26/2011_09_26_drive_0027_sync/image_02/data/0000000175.npy'
        sem_test_files = [os.path.join(
            sem_test_dir, t[:-5]+".npy") for t in test_files]
        ins_test_files = [os.path.join(
            ins_test_dir, t[:-5]+"_instance_new.npy") for t in test_files]

        test_files = [os.path.join(dataset_dir, t[:-1]) for t in test_files]
    ...
    model = SIGNetModel(opt, input_uint8, None, None,
                        [input_sem_uint8,  input_sem_map_uint8,
                            input_sem_mask_uint8,  input_sem_edge_uint8], NONES,
    ...
    with tf.Session(config=config) as sess:
        ...
        for t in range(0, len(test_files), opt.batch_size):
            ...
            inputs = np.zeros((opt.batch_size, opt.img_height,
                               opt.img_width, 3), dtype=np.uint8)

            inputs_sem = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, opt.sem_num_class), dtype=np.uint8)
            inputs_sem_map = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)
            inputs_sem_mask = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)
            inputs_sem_edge = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)

            inputs_ins0 = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, opt.ins_num_class), dtype=np.uint8)
            inputs_ins0_map = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)
            inputs_ins0_edge = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)
            
            inputs_ins1_edge = np.zeros(
                (opt.batch_size, opt.img_height, opt.img_width, 1), dtype=np.uint8)
            ...
            for b in range(opt.batch_size):
                ...
                if opt.sem_assist and opt.sem_as_feat:
                    ...
                    scaled_sem_im = sem_im.resize(
                        (opt.img_width, opt.img_height), pil.NEAREST)
                    # TODO scaled_sem_im shape (128,416)
                    scaled_sem_im = np.array(scaled_sem_im)
                    # TODO scaled_sem_im_1c shape (128,416,1)
                    scaled_sem_im_1c = np.expand_dims(scaled_sem_im, -1)

                    # True
                    if opt.one_hot_sem_feat:
                        # [128, 416, 19]
                        sem_oh_im = np.zeros(
                            list(scaled_sem_im.shape)+[opt.sem_num_class]) 
                        # TODO one-hot encoding
                        sem_oh_im[list(np.indices(
                            sem_oh_im.shape[:-1]))+[scaled_sem_im]] = 1
                        inputs_sem[b] = np.array(sem_oh_im)
                    else:
                        if opt.sem_mask_feature:
                            if opt.sem_mask_pattern == "foreground":
                                inputs_sem[b] = (
                                    scaled_sem_im_1c > 10).astype(np.uint8)
                            else:
                                inputs_sem[b] = (
                                    scaled_sem_im_1c <= 10).astype(np.uint8)
                        # TODO scaled_sem_im_1c shape (128,416,1)
                        elif opt.sem_edge_feature:
                            x = np.pad((scaled_sem_im_1c[:, :-1, :] != scaled_sem_im_1c[:, 1:, :]).astype(
                                np.uint8), ((0, 0), (0, 1), (0, 0)), 'constant')
                            y = np.pad((scaled_sem_im_1c[:-1, :, :] != scaled_sem_im_1c[1:, :, :]).astype(
                                np.uint8), ((0, 1), (0, 0), (0, 0)), 'constant')
                            inputs_sem[b] = x+y
                        else:
                ...
                if opt.ins_assist:
                    ...
                    scaled_ins0_im = ins0_im.resize(
                        (opt.img_width, opt.img_height), pil.NEAREST)
                    # TODO scaled_ins0_im shape (128,416)
                    scaled_ins0_im = np.array(scaled_ins0_im)
                    # TODO scaled_ins0_im_1c shape (128,416,1)
                    scaled_ins0_im_1c = np.expand_dims(scaled_ins0_im, -1)

                    scaled_ins1_im = ins1_im.resize(
                        (opt.img_width, opt.img_height), pil.NEAREST)
                    # TODO scaled_ins0_im shape (128,416)
                    scaled_ins1_im = np.array(scaled_ins1_im)
                    # TODO scaled_ins1_im_1c shape (128,416,1)
                    scaled_ins1_im_1c = np.expand_dims(scaled_ins1_im, -1)

                    ins0_oh_im = np.zeros(
                        list(scaled_ins0_im.shape)+[opt.ins_num_class])
                    # TODO one-hot encoding
                    ins0_oh_im[list(np.indices(
                        ins0_oh_im.shape[:-1]))+[scaled_ins0_im]] = 1
                    ...
                    # False
                    if opt.ins0_edge_explore:
                        x = np.pad((scaled_ins0_im_1c[:, :-1, :] != scaled_ins0_im_1c[:, 1:, :]).astype(
                            np.uint8), ((0, 0), (0, 1), (0, 0)), 'constant')
                        y = np.pad((scaled_ins0_im_1c[:-1, :, :] != scaled_ins0_im_1c[1:, :, :]).astype(
                            np.uint8), ((0, 1), (0, 0), (0, 0)), 'constant')
                        inputs_ins0_edge[b] = x+y
                    
                    # True
                    if opt.ins1_edge_explore:
                        x = np.pad((scaled_ins1_im_1c[:, :-1, :] != scaled_ins1_im_1c[:, 1:, :]).astype(
                            np.uint8), ((0, 0), (0, 1), (0, 0)), 'constant')
                        y = np.pad((scaled_ins1_im_1c[:-1, :, :] != scaled_ins1_im_1c[1:, :, :]).astype(
                            np.uint8), ((0, 1), (0, 0), (0, 0)), 'constant')
        ...
        np.save(os.path.join(opt.output_dir, 'model.npy'), pred_all)