...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        if opt.train_lite:
            file_list = self.format_file_list(
                opt.dataset_dir, opt.filelist_dir, 'train_lite')
        else:
            file_list = self.format_file_list(
                opt.dataset_dir, opt.filelist_dir, 'train')
        ...
        if opt.sem_assist:
            ...
            if opt.load_from_raw:
                sem_seq = tf.reshape(tf.decode_raw(sem_contents, tf.uint8), [
                                     1, opt.img_height, (opt.num_source+1) * opt.img_width])
            else:
                sem_seq = tf.py_func(read_npy_file, [sem_keys], [tf.uint8, ])
                sem_seq = tf.reshape(
                    sem_seq, [1, opt.img_height, (opt.num_source+1) * opt.img_width])
        ...
        if opt.ins_assist:
            ...
            if opt.load_from_raw:
                ...
                ins_seq = tf.reshape(
                    ins_seq, [1, opt.img_height, (opt.num_source+1) * opt.img_width, 2])
        ...
        if opt.sem_assist:
            ...
            tgt_sem, src_sem_stack = self.data_aug(
                tgt_sem, src_sem_stack, aug_params, "bilinear")
            ...
            if self.opt.sem_edge_explore:
                tgt_sem_edge, src_sem_edge_stack = \
                    self.data_aug(tgt_sem_edge, src_sem_edge_stack,
                                  aug_params, "bilinear")
        ...
        if opt.ins_assist:
            ...
            tgt_ins0, src_ins0_stack = self.data_aug(
                tgt_ins0, src_ins0_stack, aug_params, "bilinear")
            ...
            if self.opt.ins0_edge_explore:
                tgt_ins0_edge, src_ins0_edge_stack = \
                    self.data_aug(tgt_ins0_edge, src_ins0_edge_stack,
                                  aug_params, "bilinear")
            ...
            if self.opt.ins1_edge_explore:
                tgt_ins1_edge, src_ins1_edge_stack = \
                    self.data_aug(tgt_ins1_edge, src_ins1_edge_stack,
                                  aug_params, "bilinear")
        ...
        return tgt_image, src_image_stack, intrinsics, \
            ...
            [src_ins0_stack, src_ins0_map_stack,
    ...
    def format_file_list(self, data_root, filelist_root, split):
        ...
        frame_ids = [x.zfill(10) for x in frame_ids]
        ...
        if self.opt.sem_assist:
            ...
            if self.opt.load_from_raw:
                # @qihao
                sem_data_root = data_root.replace(
                    "kitti_raw_eigen", "kitti_raw_eigen_sem")
                sem_file_list = [os.path.join(sem_data_root, subfolders[i],
        ...
        if self.opt.ins_assist:
            ...
            if self.opt.load_from_raw:
                # @qihao
                ins_data_root = data_root.replace(
                    "kitti_raw_eigen", "kitti_raw_eigen_ins")
                ins_file_list = [os.path.join(ins_data_root, subfolders[i],
                                              frame_ids[i] + '.raw') for i in range(len(frames))]
            else:
                # @qihao
                ins_data_root = data_root.replace(
        ...
        if self.opt.sem_assist:
            ...
            # print(sem_file_list[0])
            # /userhome/34/h3567721/dataset/kitti/kitti_raw_eigen_sem/2011_09_30_drive_0034_sync_03/0000000558.npy
        ...
        if self.opt.ins_assist:
            ...
            # print(ins_file_list[0])
            # /userhome/34/h3567721/dataset/kitti/kitti_raw_eigen_ins/2011_09_30_drive_0034_sync_03/0000000558.npy
    ...
    # 4*128*(416*3=1248) X {0,1,..,18} =>  4*128*1248*c
    def get_sem_mask_batch(self, sem_seq):
    ...
    # 4*128*(416*3=1248) X {0,1,..,18} =>  4*128*1248*c
    def get_sem_edge_batch(self, sem_seq):

        def some_f(seg):
            x = tf.pad(
                1-tf.cast(tf.equal(seg[:, :, :-1], seg[:, :, 1:]), dtype=tf.uint8), [[0, 0], [0, 0], [0, 1]])
            y = tf.pad(
                1-tf.cast(tf.equal(seg[:, :-1, :], seg[:, 1:, :]), dtype=tf.uint8), [[0, 0], [0, 1], [0, 0]])
            return x + y
