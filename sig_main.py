...
flags.DEFINE_boolean("enable_batch_norm",        True,
                     "Shall we enable batch_norm configs")
flags.DEFINE_boolean("batch_norm_is_training",   True,
                     "Set the flag for slim.batch_norm is_training")

# TODO debug flags to help debugging
flags.DEFINE_boolean("train_lite",               False,
                     "Train on light weight dataset")  # for quick debug
flags.DEFINE_integer("print_interval",             100,
                     "Interval for printing things out")  # for debugging

# TODO tensorboard visualization
flags.DEFINE_string("summary_dir",                 "",
                    "Directory name to save the summaries")
flags.DEFINE_integer("save_summ_freq",            100,
                     "Save the TF summary every save_summ_freq iterations")
# see https://www.tensorflow.org/api_docs/python/tf/summary/image
flags.DEFINE_integer("max_outputs",                 4,
                     "How many images per mini-batch we want to save")

# TODO regularization
flags.DEFINE_boolean("use_regularization",       False,
                     "Whether or not to use regularization term")

# TODO semantic-related flags to help depth estimation
flags.DEFINE_boolean("sem_assist",               False,
                     "Add semantic into account")
flags.DEFINE_boolean("load_from_raw",             True,
                     "Load from raw binary format")
flags.DEFINE_integer("sem_num_class",               19,
                     "Num of semantic classes")
flags.DEFINE_boolean("sem_as_loss",              False,
                     "Shall use semantic loss in loss function")
flags.DEFINE_boolean("sem_as_feat",              False,
                     "Shall add semantic as a feature in depth net")
flags.DEFINE_boolean("one_hot_sem_feat",         True,
                     "Shall we use one hot version of sem as input")
flags.DEFINE_float("sem_feat_weight",               .0,
                   "The weight of semantic feature in the network")
flags.DEFINE_boolean("fixed_posenet",            False,
                     "Shall we fixed the weight for posenet?")
flags.DEFINE_string("sem_test_kitti_dir",           "",
                    "Dir for semantic kitti testset")
flags.DEFINE_boolean("sem_nn_pyramid",           False,
                     "Use nearest neighbor to generate sem pyramid")
flags.DEFINE_boolean("sem_nn_warp",              False,
                     "Use nearest neighbor to execute flow warp")

# TODO explorations about the semantic loss: `sem_as_loss`==True
flags.DEFINE_boolean("sem_warp_explore",         False,
                     "Warp the semantic and compute the loss")
flags.DEFINE_string("sem_warp_function",            "",
                    "Loss function for semantic warping loss")
flags.DEFINE_float("sem_warp_weight",              0.0,
                   "The weight for semantic warping loss")
flags.DEFINE_boolean("sem_mask_explore",         False,
                     "Extact the mask and guide the img warp loss")
flags.DEFINE_boolean("sem_mask_feature",         False,
                     "Extact the mask and use as sem features, sem_mask_explore=True")
flags.DEFINE_string("sem_mask_pattern",             "",
                    "Patterns to extract the mask")
flags.DEFINE_string("sem_mask_function",            "",
                    "Functions for the guidance to the img warp loss")
flags.DEFINE_float("sem_mask_weight",              0.0,
                   "The weight for this img warp loss")
flags.DEFINE_boolean("sem_edge_explore",         False,
                     "Extact the edge and guide the smooth term")
flags.DEFINE_boolean("sem_edge_feature",         False,
                     "Extact the edge and use as sem features, sem_edge_explore=True")
flags.DEFINE_string("sem_edge_pattern",             "",
                    "Patterns to extract the edge")
flags.DEFINE_string("sem_edge_function",            "",
                    "Functions for the guidance to the smooth term")
flags.DEFINE_float("sem_edge_weight",              0.0,
                   "The weight for this smooth term")
flags.DEFINE_boolean("data_aug_cast",             True,
                     "adapt to previous version, but try to avoid this in future version")

flags.DEFINE_float("lighting_factor",         1.0,       "lighting factor")
flags.DEFINE_boolean("use_sem_weight_decay",    False,
                     "Adapt sem weight decay for sem features among training")
flags.DEFINE_string("sem_feat_struct",      "channel",
                    "Sem network type: channel or branch wise")
flags.DEFINE_float("weight_init_value",           1.0,
                   "Initial value for sem features. May 1.0 for type-I and 0.5 for type-II")
flags.DEFINE_string("weight_decay_rule",           "",
                    "1) Finally comes to zero 2) at some drop speed defined")

# TODO explorations about semantic-guided depth prediction network
flags.DEFINE_boolean("add_segnet",              False,
                     "To enable the transfer network to learn multi scale semantic segmentation from depth")
flags.DEFINE_string("transfer_network_structure",  "",
                    "The structure for that transfer network")
flags.DEFINE_float("sem_seg_weight",              0.0,
                   "The weight for semantic segmentation guidance loss weight")
flags.DEFINE_float("ins0_seg_weight",             0.0,
                   "The weight for instance class segmentation guidance loss weight")
flags.DEFINE_float("ins1_edge_seg_weight",        0.0,
                   "The weight for instance id edge segmentation guidance loss weight")
flags.DEFINE_boolean("transfer_learn_sem",         True,
                     "Use transfer network to learn semantic")
flags.DEFINE_boolean("transfer_learn_ins0",       False,
                     "Use transfer network to learn ins0")
flags.DEFINE_boolean("transfer_learn_ins1_edge",  False,
                     "Use transfer network to learn ins1 edges")

# TODO  Try to observe the variation of architectures handling semantic inputs
flags.DEFINE_boolean("block_dispnet_sem",       False,
                     "Not to receive the semantic input for dispnet")
flags.DEFINE_boolean("block_posenet_sem",       False,
                     "Not to receive the semantic input for posenet")
flags.DEFINE_boolean("new_sem_dispnet",         False,
                     "To build a new dispnet for receiving semantic input")
flags.DEFINE_boolean("new_sem_posenet",         False,
                     "To build a new posenet for receiving semantic input")

# TODO instance-related flags to help depth estimation
flags.DEFINE_boolean("ins_assist",               False,
                     "Add instance into account")
flags.DEFINE_integer("ins_num_class",               81,
                     "Maximum number of instances labels")
flags.DEFINE_boolean("ins_as_loss",              False,
                     "Shall use instance loss in loss function")
flags.DEFINE_float("ins_l2_norm_weight",           .3,
                   "The weight of ins l2-norm in loss function")
flags.DEFINE_boolean("ins_as_feat",              False,
                     "Shall add instance as a feature in depth net")

flags.DEFINE_boolean("ins0_dense_feature",        False,
                     "Shall use instance channel 0 dense info as features?")
flags.DEFINE_boolean("ins0_onehot_feature",       False,
                     "Shall we use one hot version of ins as input")
flags.DEFINE_boolean("ins0_edge_explore",         False,
                     "Shall explore instance channel 0 edge info?")
flags.DEFINE_boolean("ins0_edge_feature",         False,
                     "Shall use instance channel 0 edge info as features?")
flags.DEFINE_boolean("ins1_dense_feature",        False,
                     "Shall use instance channel 1 dense info as features?")
flags.DEFINE_boolean("ins1_onehot_feature",       False,
                     "Shall we use one hot version of ins as input")
flags.DEFINE_boolean("ins1_edge_explore",         False,
                     "Shall explore instance channel 1 edge info?")
flags.DEFINE_boolean("ins1_edge_feature",         False,
                     "Shall use instance channel 1 edge info as features?")

flags.DEFINE_string("ins_train_kitti_dir",
                    "../../data/kitti_eigen_new_instance_labels_acc_only/", "kitt inst train path?")
flags.DEFINE_string("ins_test_kitti_dir",
                    "../../data/test_files_eigen_semantic_acc_only/",     "kitt inst testpath?")

flags.DEFINE_float("ins_feat_weight",               .0,
                   "The weight of instance feature in the network")
flags.DEFINE_boolean("ins_nn_pyramid",           False,
                     "Use nearest neighbor to generate ins pyramid")
flags.DEFINE_boolean("ins_nn_warp",              False,
                     "Use nearest neighbor to execute flow warp")
...
flags.DEFINE_integer("max_to_keep",               5000,
...
flags.DEFINE_float("alpha_recon_image",           0.85,
                   "Alpha weight between SSIM and L1 in reconstruction loss")
...
flags.DEFINE_float("flow_consistency_alpha",       3.0,
                   "Alpha for flow consistency check")
flags.DEFINE_float("flow_consistency_beta",       0.05,
                   "Beta for flow consistency check")
...
flags.DEFINE_integer("num_source",                   2,
                     "Number of source images")
flags.DEFINE_integer("num_scales",                   4,
...
def train():
    ...
    with tf.Graph().as_default():
            ...
            for i in range(opt.num_gpus):
                with tf.device('/gpu:{:d}'.format(i)):
                    with tf.name_scope('gpu{:d}'.format(i)):
                        ...
                        # Build Model
                        model = SIGNetModel(opt, tgt_image, src_image_stack, intrinsics,
                                            tgt_sem_tuple, src_sem_stack_tuple, tgt_ins_tuple, src_ins_stack_tuple)
                            ...
                            if opt.mode == 'train_flow' and opt.flownet_type == "residual":
                                ...
                                if opt.mode == 'train_rigid' and opt.fixed_posenet:
                                    ...
                                    if opt.new_sem_dispnet:
                                        train_vars = tf.get_collection(
                                            tf.GraphKeys.TRAINABLE_VARIABLES, "depth_sem_net")
                                    else:
                                        train_vars = tf.get_collection(
                                            tf.GraphKeys.TRAINABLE_VARIABLES, "depth_net")
                                else:
                                    # here
                                    train_vars = [
        ...
        tf.summary.scalar('unit_rigid_warp_loss', rigid_warp_loss /
                          (opt.rigid_warp_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
        ...
        tf.summary.scalar('unit_disp_smooth_loss', disp_smooth_loss /
                          (opt.disp_smooth_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
        ...
        if opt.sem_as_loss:
            ...
            if opt.sem_warp_explore:
                ...
                tf.summary.scalar('unit_sem_warp_loss', model.sem_warp_loss /
                                  (opt.sem_warp_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
            # True
            if opt.sem_mask_explore:
                ...
                tf.summary.scalar('unit_sem_mask_loss', model.sem_mask_loss /
                                  (opt.sem_mask_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
            ...
            if opt.sem_edge_explore:
                ...
                tf.summary.scalar('unit_sem_edge_loss', model.sem_edge_loss /
                                  (opt.sem_edge_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
        ...
        if opt.sem_assist and opt.add_segnet:
            ...
            tf.summary.scalar('unit_sem_seg_loss', model.sem_seg_loss /
                              (opt.sem_seg_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
            ...
            tf.summary.scalar('unit_ins0_seg_loss', model.ins0_seg_loss /
                              (opt.ins0_seg_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
            ...
            tf.summary.scalar('unit_ins1_edge_seg_loss', model.ins1_edge_seg_loss /
                              (opt.ins1_edge_seg_weight+tf.convert_to_tensor(1e-8, dtype=tf.float32)))
...
def main(_):
    ...
    opt.num_source = opt.seq_length - 1  # 3-1=2
    opt.num_scales = 4
